...
const AdminController = {
    createEmployee : async (req, res) => {
        try {
            const {email, roleId, baseSalary, daySalary} = req.body
            if(!email || !roleId || !baseSalary || !daySalary) return res.status(500).json({message : "Data is not complete"})
    ...
    employeeForm : async (req, res) => {
        try {
            ...
            if(!password || !fullName || !username || !birthday) return res.status(500).json({message : "Data is not complete"})
